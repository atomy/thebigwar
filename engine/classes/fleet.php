			...
			if($type == 1 && !$back)
			{
				...
				else
				{
					...
					if(!$start_user->checkPlanetCount())
					{
						...
						{
							$message->subject('Besiedelung von '.$next_target_nt.' fehlgeschlagen');
							$message->text("Ihre Flotte erreicht den Planeten ".$next_target_nt." und will mit der Besiedelung anfangen. Als Sie jedoch Ihren Zentralcomputer um BestÃ¤tigung fÃ¼r die Besiedelung bittet, kommt dieser durcheinander, da Sie schon so viele Planeten haben und er nicht so viele gleichzeitig kontrollieren kann, und schickt in Panik Ihrer Flotte das Signal zum RÃ¼ckflug.");
							$message->addUser($first_user, 5);
					...
					fwrite($fo, date('Y-m-d, H:i:s')." arriveAtNextTarget() -- Sammeln Ende. Flotten-ID:  ".$this->getName()."\n");
						...
						case 3: # Angriff
							...
							if($winner == 1)
							{
								...
								foreach($users_keys as $username)
								{
									...
									if($exp2[0] == $username)
									{
										$messages[$username] .= "\n<p class=\"rohstoffe-erbeutet andere\">Die Ã¼berlebenden Angreifer haben ".ths($ress_max[0])." Carbon, ".ths($ress_max[1])." Aluminium, ".ths($ress_max[2])." Wolfram, ".ths($ress_max[3])." Radium und ".ths($ress_max[4])." Tritium erbeutet.</p>\n";
									}
								}
							}
						...
						case 4: # Transport
							...
							if(!isset($this->raw[6][1]))
								$message_text = array(
								$target_owner => "Ein Transport erreicht Ihren Planeten \xe2\x80\x9e".$target_user->planetName()."\xe2\x80\x9c (".$next_target_nt."). Folgende Spieler liefern GÃ¼ter ab:\n"
								);
							else
							$message_text = array(
							...
							if(!isset($this->raw[6][0])) $this->raw[6][0] = 0;
								...
								foreach($this->raw[1] as $username=>$data)
								{
									...
									if($write_this_username) $message_text[$username] = "Ihre Flotte erreicht den Planeten \xe2\x80\x9e".$target_user->planetName()."\xe2\x80\x9c (".$next_target_nt.", Eigent\xc3\xbcmer: ".$target_owner.") und liefert folgende GÃ¼ter ab:\n";
