...
class Fleet extends Dataset
{
    ...
    function getArrivalTime( )
    {
        ...
        if ( ! isset( $this->raw[6][0] ) )
            $this->raw[6][0] = 0;
        if ( isset( $this->raw[7][0] ) && $this->raw[7][0] > time() && $this->raw[6][0] !== - 1 )
            return $this->raw[7][0];
        if ( isset( $this->raw[7][1] ) && $this->raw[6][0] == - 1 && $this->raw[7][1] > time() )
            return $this->raw[7][1];
    ...
    function subForeignFleet( $id, $count2, $user )
    {
        ...
        if ( ! isset( $that->raw[1][$user] ) )
            return false;
        
        $keys = array_keys( $that->raw[1] );
        $first = ! array_search( $user, $keys );
        if ( isset( $that->raw[1][$user][0][$id] ) )
            $that->raw[1][$user][0][$id] = $count2;
    ...
    function addUser( $user, $from, $factor = 1 )
    {
        ...
        if ( $factor <= 0 )
            $factor = 0.01;
        $this->raw[1][$user] = array( array(), # Flotten
...
//            echo "BLERK\n";
    ...
    function getTime( $i )
    {
        if ( ! $this->status || ( ! isset( $this->raw[0][$i] ) && ! isset( $this->raw[0][$i . 'T'] ) ) )
            return false;
        if ( count( $this->raw[1] ) <= 0 )
            return false;
    ...
    {
        if ( ! $this->status || ! isset( $this->raw[1][$user] ) || count( $this->raw[1][$user] ) <= 0 )
            return false;
    ...
    function callBack( $user, $immediately = false )
    {
        ...
        {
            if ( substr( $next_t, - 1 ) == 'T' )
                $next_t = substr( $next_t, 0, - 1 );
            $back_tritium += $this->getTritium( $userres, $prev_t, $next_t );
        }
        if ( $from_t == $start )
            $tritium1 = 0;
        else
            $tritium1 = $this->getTritium( $user, $from, $start );
        ...
        if ( $holdtime == - 1 )
        {
            ...
            $new_raw = array( array( $start => array( $this->raw[0][$to_t][0], 1 ) ), array( $userres => $this->raw[1][$user] ), time(), array( $to_t => $this->raw[0][$to_t] ) );
        
        }
        else
        {
            $new_raw = array( array( $start => array( $this->raw[0][$to_t][0], 1 ) ), array( $userres => $this->raw[1][$user] ), ( time() + $back_time ) - $time2, array( $to_t => $this->raw[0][$to_t] ) );
        }
    ...
    {
        if ( ! $this->status || $this->started() || ! isset( $this->raw[1][$user] ) )
            return false;
...
', ths( $trans_total ), ths( $rtrans[0] ), ths( $rtrans[1] ), ths( $rtrans[2] ), ths( $rtrans[3] ), ths( $tr_verbl[0] ), ths( $tr_verbl[1] ), ths( $tr_verbl[2] ), ths( $tr_verbl[3] ) ) );
                    ...
                    case 3: # Angriff
                                    ...
                                    foreach ( $that->raw[1] as $username1 => $info )
                                    {
                                        ...
                                        foreach ( $that->raw[1][$username1][0] as $id => $anzahl )
                                        {
                                            ...
                                            else
                                            {
                                                if ( $exp[0] == $username1 )
                                                    continue;
                                                if ( ! isset( $that->raw[1][$exp[0]] ) )
                                                    $that->raw[1][$exp[0]] = $that->raw[1][$username1];
                                                else
                                                    $that->addFleet( $id, $anzahl, $exp[0] );
                                                if ( $exp[0] !== $username1 )
                                                    unset( $that->raw[1][$username1] );
                                            }
                                        }
                        ...
                        if ( $count > 0 )
                        {
                            ...
                            foreach ( $angreifer2 as $username2 => $ida2 )
                            {
                                ...
                                foreach ( $angreifer3 as $username3 => $ida3 )
                                {
                                    ...
                                    if ( ! array_key_exists( $exp[0], $angreifer2 ) )
                                        unset( $angreifer3[$username3] );
                                    ...
                                    if ( $exp[0] == $username2 )
                                    {
                                        ...
                                        foreach ( $ida3 as $id3 => $anzahl3 )
                                        {
                                            ...
                                            if ( ! array_key_exists( $id3, $angreifer2[$exp[0]] ) )
                                                unset( $angreifer3[$username3][$id3] );
                                                
                                            #Anzahl der verschiedenen IDs zusammenzaehlen
                                            if ( isset( $angreifer3[$username3][$id3] ) )
                                                $gesamtangreifer3[$username2][$id3] += $anzahl3;
                                        }
                                    }
                                }
                        ...
                        if ( isset( $urverteidiger[$target_owner] ) )
                            unset( $urverteidiger[$target_owner] );
                    ...
                    case 4: # Transport
                        ...
                        if ( ! isset( $this->raw[6][1] ) )
                            ...
                            $message_text = array( $target_owner => "Ein Transport erreicht Ihren Planeten \xe2\x80\x9e" . $target_user->planetName() . "\xe2\x80\x9c (" . $next_target_nt . "). Die Flotte hat das Transportgut an Board behalten.\n", $first_user => "Ein Transport erreicht den Planeten \xe2\x80\x9e" . $target_user->planetName() . "\xe2\x80\x9c (" . $next_target_nt . "). Die Flotte hat das Transportgut an Board behalten.\n" );
                        ...
                        else
                        {
                            ...
                            switch ( $diff )
                            {
                                case 5: # Roboter und Fremdflotte zeigen
                                    ...
                                    {
                                        if ( $target_user->getItemLevel( $id, 'roboter' ) <= 0 )
                                            continue;
                                ...
                                case 4: # Forschung zeigen
                                    ...
                                    {
                                        if ( $target_user->getItemLevel( $id, 'forschung' ) <= 0 )
                                            continue;
                                ...
                                case 3: # Schiffe und Verteidigungsanlagen anzeigen
                                    ...
                                    {
                                        if ( $target_user->getItemLevel( $id, 'schiffe' ) <= 0 )
                                            continue;
                                    ...
                                    {
                                        if ( $target_user->getItemLevel( $id, 'verteidigung' ) <= 0 )
                                            continue;
                                ...
                                case 2: # Gebaeude anzeigen
                                    ...
                                    {
                                        if ( $target_user->getItemLevel( $id, 'gebaeude' ) <= 0 )
                                            continue;
            ...
            if ( $further )
            {
                ...
                foreach ( $users as $user )
                {
                    ...
                    if ( $new_fleet->create() && $further == true )
                    {
                        ...
                        $new_fleet->setRaw( array( array( $that->raw[1][$exp[0]][1] => array( $type, true ) ), array( $exp[0] => $this->raw[1][$user] ), time(), array( $next_target => array( $type, false ) ) ) );
        ...
        else
        {
            ...
            foreach ( $this->raw[1] as $username => $move_info )
            {
                ...
                if ( $username == $owner )
                {
                    ...
                    {
                        if ( isset( $schiffe_own[$id] ) )
                            $schiffe_own[$id] += $count;
                        else
                            $schiffe_own[$id] = $count;
                    }
                ...
                else
                {
                    if ( ! $besiedeln )
                    {
                        ...
                        foreach ( $move_info[0] as $id => $count )
                        {
                            if ( isset( $schiffe_other[$username][$id] ) )
                                $schiffe_other[$username][$id] += $count;
                            else
                                $schiffe_other[$username][$id] = $count;
                        }
                    }
...
function battle( $angreifer, $verteidiger )
{
    ...
    if ( count( $angreifer ) < 0 || count( $verteidiger ) < 0 )
        return false;
    ...
    elseif ( $angreifer_spiotech > $verteidiger_spiotech )
    {
    ...
    foreach ( $verteidiger as $name => $ids )
    {
        ...
        if ( count( $ids ) <= 0 )
            unset( $verteidiger[$name] );
        else
            $verteidiger[$name] = $ids;
    }
    ...
    for ( $runde = 1; $runde <= 20; $runde ++ )
    {
        ...
        while ( $staerke > 0 )
        {
            ...
            if ( $this_shield > $aff_staerke ) #
            {
                ...
                if ( $remdiff >= 1 )
                    $floor_diff += floor( $remdiff );
                if ( $remdiff >= 1 )
                    $floordiffgerundet[$att_user][$att_id] += floor( $remdiff );
    ...
    foreach ( $angreifer_anfang as $a => $i )
    {
        ...
        $p = 0;
        if ( isset( $angreifer_punkte[$a] ) )
            $p = $angreifer_punkte[$a];
    ...
    foreach ( $verteidiger_anfang as $v => $i )
    {
        ...
        $p = 0;
        if ( isset( $verteidiger_punkte[$v] ) )
            $p = $verteidiger_punkte[$v];
    ...
    {
        $user_obj = Classes::User( $user );
        $user_obj->addScores( 6, ( $angreifer_new_erfahrung * $angreiferfaktor[$user] ) );
    }
    ...
    if ( $verteidigerkomplett == 0 )
        $verteidigerkomplett = 1;
    foreach ( $verteidigergesamt as $name => $anzahl )
    ...
    {
        $user_obj = Classes::User( $user );
        $user_obj->addScores( 6, ( $verteidiger_new_erfahrung * $verteidigerfaktor[$user] ) );
    }
