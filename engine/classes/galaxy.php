...
class Galaxy
{
    ...
    private function seekSystem( $system )
    {
        ...
        $pos = ( $system - 1 ) * 1655;
        if ( $this->filesize < $pos + 1655 )
            return false; # System existiert nicht
    ...
    function getPlanetsCount( $system )
    {
        ...
        if ( ! isset( $this->cache['getPlanetsCount'] ) )
            $this->cache['getPlanetsCount'] = array();
    ...
    function _getPlanetOwner( $system, $planet )
    {
        ...
        if ( ! isset( $this->cache['getPlanetOwner'] ) )
            $this->cache['getPlanetOwner'] = array();
        if ( ! isset( $this->cache['getPlanetOwner'][$system] ) )
            $this->cache['getPlanetOwner'][$system] = array();
    ...
    function _setPlanetOwner( $system, $planet, $owner )
    {
        ...
        $planets_count = $this->getPlanetsCount( $system );
        if ( ! $planets_count || $planet > $planets_count || $planet < 1 )
            return false;
        ...
        fseek( $this->file_pointer, 35 + ( $planet - 1 ) * 24, SEEK_CUR );
        if ( strlen( $owner ) > 0 && ! fwrite( $this->file_pointer, $owner ) )
            return false;
        if ( strlen( $owner ) < 24 )
            fwrite( $this->file_pointer, str_repeat( ' ', 24 - strlen( $owner ) ) );
    ...
    function getPlanetName( $system, $planet )
    {
        ...
        if ( ! isset( $this->cache['getPlanetName'] ) )
            $this->cache['getPlanetName'] = array();
        if ( ! isset( $this->cache['getPlanetName'][$system] ) )
            $this->cache['getPlanetName'][$system] = array();
    ...
    function setPlanetName( $system, $planet, $name )
    {
        ...
        if ( strlen( $name ) > 0 && ! fwrite( $this->file_pointer, $name ) ) {
            return false;
        }
        
        if ( strlen( $name ) < 24 ) {
            fwrite( $this->file_pointer, str_repeat( ' ', 24 - strlen( $name ) ) );
        }
    ...
    function getPlanetOwnerAlliance( $system, $planet )
    {
        ...
        if ( ! isset( $this->cache['getPlanetOwnerAlliance'] ) )
            $this->cache['getPlanetOwnerAlliance'] = array();
        if ( ! isset( $this->cache['getPlanetOwnerAlliance'][$system] ) )
            $this->cache['getPlanetOwnerAlliance'][$system] = array();
    ...
    function setPlanetOwnerAlliance( $system, $planet, $alliance )
    {
        ...
        fseek( $this->file_pointer, 1475 + ( $planet - 1 ) * 6, SEEK_CUR );
        if ( strlen( $alliance ) > 0 && ! fwrite( $this->file_pointer, $alliance ) )
            return false;
        if ( strlen( $alliance ) < 6 )
            fwrite( $this->file_pointer, str_repeat( ' ', 6 - strlen( $alliance ) ) );
    ...
    function getPlanetSize( $system, $planet )
    {
        ...
        if ( ! isset( $this->cache['getPlanetSize'] ) )
            $this->cache['getPlanetSize'] = array();
        if ( ! isset( $this->cache['getPlanetSize'][$system] ) )
            $this->cache['getPlanetSize'][$system] = array();
    ...
    function setPlanetSize( $system, $planet, $size )
    {
        ...
        if ( ! fwrite( $this->file_pointer, chr( $byte1 ) . chr( $byte2 ) ) )
            return false;
    ...
    function resetPlanet( $system, $planet )
    {
        ...
        return ( $this->setPlanetName( $system, $planet, '' ) && $this->_setPlanetOwner( $system, $planet, '' ) && $this->setPlanetOwnerAlliance( $system, $planet, '' ) && $this->setPlanetSize( $system, $planet, rand( 100, 500 ) ) );
    }
