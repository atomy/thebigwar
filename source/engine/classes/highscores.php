...
class Highscores
{
    ...
    function updateAlliance( $tag, $scores_average = false, $scores_total = false, $members_count = false )
    {
        ...
        if ( $exists ) {
            ...
            $set = array();
            if ( $scores_average !== false )
                $set[] = "scores_average = '" . sqlite_escape_string( $scores_average ) . "'";
            if ( $scores_total !== false )
                $set[] = "scores_total = '" . sqlite_escape_string( $scores_total ) . "'";
            if ( $members_count !== false )
                $set[] = "members_count = '" . sqlite_escape_string( $members_count ) . "'";
            $query .= implode( ', ', $set );
            $query .= " WHERE tag = '" . sqlite_escape_string( $tag ) . "';";
        }
        else
            $query = "INSERT INTO highscores_alliances ( tag, scores_average, scores_total, members_count ) VALUES ( '" . sqlite_escape_string( $tag ) . "', '" . sqlite_escape_string( $scores_average ) . "', '" . sqlite_escape_string( $scores_total ) . "', '" . sqlite_escape_string( $members_count ) . "' );";
    ...
    function getList( $type, $from, $to, $sort_field = false )
    {
        ...
        elseif ( ! in_array( $sort_field, $allowed_sort_fields[$type] ) )
            return false;
