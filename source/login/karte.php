...
// <![CDATA[
    ...
    function change_system(new_system)
    {
        ...
        for(i=1; i<sinfo.length; i++)
        {
            ...
            {
                if(sinfo[i]['owner'] == '<?php echo str_replace("'", "\\'", $_SESSION['username'])?>')
            ...
            if(sinfo[i]['owner'])
            {
                if(sinfo[i]['alliance'])
                {
                    ...
                    if(sinfo[i]['alliance'] == '<?php echo str_replace("'", "\\'", $me->allianceTag())?>')
                    ...
                    new_alliance2.href = 'help/allianceinfo.php?alliance='+encodeURIComponent(sinfo[i]['alliance'])+'&<?php echo urlencode(session_name()).'='.urlencode(session_id())?>';
                ...
                playername2.href = 'help/playerinfo.php?player='+encodeURIComponent(sinfo[i]['owner'])+'&<?php echo urlencode(session_name()).'='.urlencode(session_id())?>';
            ...
            new_td.firstChild.firstChild.firstChild.href = 'flotten.php?action_galaxy='+encodeURIComponent(system_split[0])+'&action_system='+encodeURIComponent(system_split[1])+'&action_planet='+encodeURIComponent(i)+'&<?php echo urlencode(session_name()).'='.urlencode(session_id())?>';
            ...
            if(<?php echo $sc_jcheck?>)
            {
                ...
                new_el2.href = 'flotten.php?action=shortcut&action_galaxy='+encodeURIComponent(system_split[0])+'&action_system='+encodeURIComponent(system_split[1])+'&action_planet='+encodeURIComponent(i)+'&<?php echo urlencode(session_name()).'='.urlencode(session_id())?>';
...
?>
                ...
                if(sinfo[i]['flag'] != 'U')
                {
                    ...
                    new_el1.appendChild(new_el2 = document.createElement('a'));
                    new_el2.href = 'flotten.php?action=spionage&action_galaxy='+encodeURIComponent(system_split[0])+'&action_system='+encodeURIComponent(system_split[1])+'&action_planet='+encodeURIComponent(i)+'&<?php echo urlencode(session_name()).'='.urlencode(session_id())?>';
...
?>
                ...
                if(sinfo[i]['owner'])
                {
                    ...
                    new_el1.appendChild(new_el2 = document.createElement('a'));
                    new_el2.href = 'nachrichten.php?to='+encodeURIComponent(sinfo[i]['owner'])+'&<?php echo urlencode(session_name()).'='.urlencode(session_id())?>';
...
?>
                ...
                if(!sinfo[i]['owner'])
                {
                    ...
                    new_el1.appendChild(new_el2 = document.createElement('a'));
                    new_el2.href = 'flotten.php?action=besiedeln&action_galaxy='+encodeURIComponent(system_split[0])+'&action_system='+encodeURIComponent(system_split[1])+'&action_planet='+encodeURIComponent(i)+'&<?php echo urlencode(session_name()).'='.urlencode(session_id())?>';
...
?>
                ...
                if(sinfo[i]['truemmerfeld'])
                {
                    ...
                    new_el1.appendChild(new_el2 = document.createElement('a'));
                    new_el2.href = 'flotten.php?action=sammeln&action_galaxy='+encodeURIComponent(system_split[0])+'&action_system='+encodeURIComponent(system_split[1])+'&action_planet='+encodeURIComponent(i)+'&<?php echo urlencode(session_name()).'='.urlencode(session_id())?>';
...
?>
        ...
        document.getElementById('galaxy-prev-link').href = 'karte.php?galaxy=<?php echo urlencode($prev_galaxy)?>&system='+encodeURIComponent(current_system)+'&<?php echo urlencode(session_name()).'='.urlencode(session_id())?>';
        document.getElementById('galaxy-next-link').href = 'karte.php?galaxy=<?php echo urlencode($next_galaxy)?>&system='+encodeURIComponent(current_system)+'&<?php echo urlencode(session_name()).'='.urlencode(session_id())?>';
        document.getElementById('system-prev-link').href = 'karte.php?galaxy=<?php echo urlencode($galaxy_n)?>&system='+encodeURIComponent(prev_system)+'&<?php echo urlencode(session_name()).'='.urlencode(session_id())?>';
        document.getElementById('system-next-link').href = 'karte.php?galaxy=<?php echo urlencode($galaxy_n)?>&system='+encodeURIComponent(next_system)+'&<?php echo urlencode(session_name()).'='.urlencode(session_id())?>';
    }
...
?>
<h3>Karte <span class="karte-koords">(<span id="sys"><?php echo utf8_htmlentities($galaxy_n)?>:<?php echo utf8_htmlentities($system_n)?></span>)</span></h3>
<form action="karte.php" method="get" class="karte-wahl"<?php if($me->checkSetting('ajax')){?> onsubmit="if(document.getElementById('current-galaxy-number').value=='<?php echo $galaxy_n?>'){change_system('<?php echo $galaxy_n?>:'+document.getElementById('current-system-number').value);return false;}"<?php }?>>
    <fieldset class="karte-galaxiewahl">
        ...
        <ul>
            <li><a href="karte.php?galaxy=<?php echo htmlentities(urlencode($prev_galaxy))?>&amp;system=<?php echo htmlentities(urlencode($system_n))?>&amp;<?php echo htmlentities(urlencode(session_name()).'='.urlencode(session_id()))?>" tabindex="6" accesskey="u" title="[U]" id="galaxy-prev-link">Vorige</a></li>
            <li><input type="text" name="galaxy" value="<?php echo utf8_htmlentities($galaxy_n)?>" tabindex="1" id="current-galaxy-number" /></li>
            <li><a href="karte.php?galaxy=<?php echo htmlentities(urlencode($next_galaxy))?>&amp;system=<?php echo htmlentities(urlencode($system_n))?>&amp;<?php echo htmlentities(urlencode(session_name()).'='.urlencode(session_id()))?>" tabindex="5" accesskey="x" title="[X]" id="galaxy-next-link">Nächste</a></li>
        </ul>
    </fieldset>
    <fieldset class="karte-systemwahl">
        <legend>System</legend>
        <ul>
            <li><a href="karte.php?galaxy=<?php echo htmlentities(urlencode($galaxy_n))?>&amp;system=<?php echo htmlentities(urlencode($prev_system))?>&amp;<?php echo htmlentities(urlencode(session_name()).'='.urlencode(session_id()))?>" tabindex="4" rel="prev" accesskey="o" id="system-prev-link"<?php if($me->checkSetting('ajax')){?> onclick="return sw_prev_system();"<?php }?>>V<kbd>o</kbd>rige</a></li>
            <li><input type="text" name="system" value="<?php echo utf8_htmlentities($system_n)?>" tabindex="2" id="current-system-number" /></li>
            <li><a href="karte.php?galaxy=<?php echo htmlentities(urlencode($galaxy_n))?>&amp;system=<?php echo htmlentities(urlencode($next_system))?>&amp;<?php echo htmlentities(urlencode(session_name()).'='.urlencode(session_id()))?>" tabindex="3" rel="next" accesskey="n" id="system-next-link"<?php if($me->checkSetting('ajax')){?> onclick="return sw_next_system();"<?php }?>><kbd>N</kbd>ächste</a></li>
        </ul>
    </fieldset>
    <div class="karte-wahl-absenden">
        <button type="submit" tabindex="7" accesskey="w"><kbd>W</kbd>echseln</button><input type="hidden" name="<?php echo htmlentities(session_name())?>" value="<?php echo htmlentities(session_id())?>" />
    </div>
</form>
...
<?php
        ...
        if($truemmerfeld !== false && array_sum($truemmerfeld) > 0)
            ...
            $tf_string = '';
?>
            <td class="c-planet"><?php echo utf8_htmlentities($i)?><?php echo $tf_string?></td>
<?php
        if($planet[1])
        {
?>
            <td class="c-name"><?php if($planet[3]){?><span class="allianz<?php echo ($planet[3] == $me->allianceTag()) ? ' verbuendet' : ''?>">[<a href="help/allianceinfo.php?alliance=<?php echo htmlentities(urlencode($planet[3]))?>&amp;<?php echo htmlentities(urlencode(session_name()).'='.urlencode(session_id()))?>" title="Informationen zu dieser Allianz anzeigen"><?php echo utf8_htmlentities($planet[3])?></a>]</span> <?php }?><?php echo utf8_htmlentities($planet[2])?> <span class="playername">(<a href="help/playerinfo.php?player=<?php echo htmlentities(urlencode($that_uname))?>&amp;<?php echo htmlentities(urlencode(session_name()).'='.urlencode(session_id()))?>" title="Informationen zu diesem Spieler anzeigen"><?php echo utf8_htmlentities($that_uname)?></a><?php echo $planet[4] ? ' ('.htmlspecialchars($planet[4]).')' : ''?>)</span></td>
<?php
        }
        else
        {
...
?>
            <td class="c-aktionen">
                <ul>
                    <li class="c-koordinaten-verwenden"><a href="flotten.php?action_galaxy=<?php echo htmlentities(urlencode($galaxy_n))?>&amp;action_system=<?php echo htmlentities(urlencode($system_n))?>&amp;action_planet=<?php echo htmlentities(urlencode($i))?>&amp;<?php echo htmlentities(urlencode(session_name()).'='.urlencode(session_id()))?>" title="Die Koordinaten dieses Planeten ins Flottenmenü einsetzen">Koordinaten verwenden</a></li>
<?php
        if(!in_array($galaxy_n.':'.$system_n.':'.$i, $me->getPosShortcutsList()))
        {
?>
                    <li class="c-lesezeichen"><a href="flotten.php?action=shortcut&amp;action_galaxy=<?php echo htmlentities(urlencode($galaxy_n))?>&amp;action_system=<?php echo htmlentities(urlencode($system_n))?>&amp;action_planet=<?php echo htmlentities(urlencode($i))?>&amp;<?php echo htmlentities(urlencode(session_name()).'='.urlencode(session_id()))?>" title="Die Koordinaten dieses Planeten zu den Lesezeichen hinzufügen"<?php if($me->checkSetting('ajax')){?> onclick="return fast_action(this, 'shortcut', <?php echo $galaxy_n?>, <?php echo $system_n?>, <?php echo $i?>);"<?php }?>>Lesezeichen</a></li>
<?php
        }
        ...
        if($that_uname != $_SESSION['username'])
        {
            ...
            {
?>
                    <li class="c-spionieren"><a href="flotten.php?action=spionage&amp;action_galaxy=<?php echo htmlentities(urlencode($galaxy_n))?>&amp;action_system=<?php echo htmlentities(urlencode($system_n))?>&amp;action_planet=<?php echo htmlentities(urlencode($i))?>&amp;<?php echo htmlentities(urlencode(session_name()).'='.urlencode(session_id()))?>" title="Spionieren Sie diesen Planeten aus"<?php if($me->checkSetting('ajax')){?> onclick="return fast_action(this, 'spionage', <?php echo $galaxy_n?>, <?php echo $system_n?>, <?php echo $i?>);"<?php }?>>Spionieren</a></li>
<?php
            }
            ...
            {
?>
                    <li class="c-nachricht"><a href="nachrichten.php?to=<?php echo htmlentities(urlencode($that_uname))?>&amp;<?php echo htmlentities(session_name().'='.urlencode(session_id()))?>" title="Schreiben Sie diesem Spieler eine Nachricht">Nachricht</a></li>
<?php
            }
            ...
            {
?>
                    <li class="c-besiedeln"><a href="flotten.php?action=besiedeln&amp;action_galaxy=<?php echo htmlentities(urlencode($galaxy_n))?>&amp;action_system=<?php echo htmlentities(urlencode($system_n))?>&amp;action_planet=<?php echo htmlentities(urlencode($i))?>&amp;<?php echo htmlentities(urlencode(session_name()).'='.urlencode(session_id()))?>" title="Schicken Sie ein Besiedelungsschiff zu diesem Planeten"<?php if($me->checkSetting('ajax')){?>  onclick="return fast_action(this, 'besiedeln', <?php echo $galaxy_n?>, <?php echo $system_n?>, <?php echo $i?>);"<?php }?>>Besiedeln</a></li>
<?php
            }
        }
        ...
        {
?>
                    <li class="c-truemmerfeld"><a href="flotten.php?action=sammeln&amp;action_galaxy=<?php echo htmlentities(urlencode($galaxy_n))?>&amp;action_system=<?php echo htmlentities(urlencode($system_n))?>&amp;action_planet=<?php echo htmlentities(urlencode($i))?>&amp;<?php echo htmlentities(urlencode(session_name()).'='.urlencode(session_id()))?>" title="Schicken Sie ausreichend Sammler zu diesem Trümmerfeld"<?php if($me->checkSetting('ajax')){?>  onclick="return fast_action(this, 'sammeln', <?php echo $galaxy_n?>, <?php echo $system_n?>, <?php echo $i?>);"<?php }?>>Trümmerfeld</a></li>
<?php
        }
?>
                </ul>
