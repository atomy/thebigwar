...
<?php
        ...
        {
?>
    <dt><a href="help/playerinfo.php?player=<?php=htmlspecialchars(urlencode($anfrage))?>&amp;<?php=htmlspecialchars(urlencode(session_name()).'='.urlencode(session_id()))?>" title="Informationen zu diesem Spieler anzeigen"><?php=utf8_htmlentities($anfrage)?></a></dt>
    <dd><ul>
        <li><a href="verbuendete.php?anfrage=<?php=htmlspecialchars(urlencode($anfrage))?>&amp;annehmen=1&amp;<?php=htmlspecialchars(urlencode(session_name()).'='.urlencode(session_id()))?>">Annehmen</a></li>
        <li><a href="verbuendete.php?anfrage=<?php=htmlspecialchars(urlencode($anfrage))?>&amp;annehmen=0&amp;<?php=htmlspecialchars(urlencode(session_name()).'='.urlencode(session_id()))?>">Ablehnen</a></li>
    </ul></dd>
<?php
        }
?>
...
<?php
        ...
        {
?>
    <dt><a href="help/playerinfo.php?player=<?php=htmlspecialchars(urlencode($bewerbung))?>&amp;<?php=htmlspecialchars(urlencode(session_name()).'='.urlencode(session_id()))?>" title="Informationen zu diesem Spieler anzeigen"><?php=utf8_htmlentities($bewerbung)?></a></dt>
    <dd><ul>
        <li><a href="verbuendete.php?bewerbung=<?php=htmlspecialchars(urlencode($bewerbung))?>&amp;<?php=htmlspecialchars(urlencode(session_name()).'='.urlencode(session_id()))?>">Zurückziehen</a></li>
    </ul></dd>
<?php
        }
?>
...
<?php
        ...
        {
?>
    <dt><a href="help/playerinfo.php?player=<?php=htmlspecialchars(urlencode($name))?>&amp;<?php=htmlspecialchars(urlencode(session_name()).'='.urlencode(session_id()))?>" title="Informationen zu diesem Spieler anzeigen"><?php=utf8_htmlentities($name)?></a></dt>
    <dd><ul>
        <li><a href="verbuendete.php?kuendigen=<?php=htmlspecialchars(urlencode($name))?>&amp;<?php=htmlspecialchars(urlencode(session_name()).'='.urlencode(session_id()))?>" onclick="return confirm('Wollen Sie das Bündnis mit dem Spieler <?php=utf8_jsentities($name)?> wirklich kündigen?');">Kündigen</a></li>
    </ul></dd>
<?php
        }
?>
</dl>
<form action="verbuendete.php?<?php=htmlspecialchars(urlencode(session_name()).'='.urlencode(session_id()))?>" method="post" class="buendnisse-rundschreiben" onsubmit="this.setAttribute('onsubmit', 'return confirm(\'Doppelklickschutz: Sie haben ein zweites Mal auf \u201eAbsenden\u201c geklickt. Dadurch wird die Nachricht auch ein zweites Mal abgeschickt. Sind Sie sicher, dass Sie diese Aktion durchführen wollen?\');');">
    <fieldset>
...
?>
<p class="error">
    <?php=htmlspecialchars($buendnis_error)."\n"?>
</p>
<?php
    }
?>
<form action="verbuendete.php?<?php=htmlspecialchars(urlencode(session_name()).'='.urlencode(session_id()))?>" method="post" class="buendnisse-eingehen">
    <dl>
        <dt class="c-spieler"><label for="spieler-input">Spieler</label></dt>
        <dd class="c-spieler"><input type="text" name="empfaenger" id="spieler-input" value="<?php=(isset($_POST['empfaenger']) ? utf8_htmlentities($_POST['empfaenger']) : '')?>" tabindex="4" accesskey="z" title="[Z]" /></dd>

        <dt class="c-mitteilung"><label for="mitteilung-textarea">Mitteilung</label></dt>
        <dd class="c-mitteilung"><textarea rows="5" cols="30" name="mitteilung" id="mitteilung-textarea" tabindex="5" accesskey="o" title="[O]"><?php=(isset($_POST['mitteilung']) ? preg_replace("/[\t\r\n]/e", '\'&#\'.ord(\'$0\').\';\'', utf8_htmlentities($_POST['mitteilung'])) : '')?></textarea></dd>
    </dl>
<?php
        if(!$me->userLocked())
        {
