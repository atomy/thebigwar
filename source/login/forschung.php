...
?>
<div class="item forschung" id="item-<?php echo htmlentities($id)?>">
    <h3><a href="help/description.php?id=<?php echo htmlentities(urlencode($id))?>&amp;<?php echo htmlentities(urlencode(session_name()).'='.urlencode(session_id()))?>" title="Genauere Informationen anzeigen"><?php echo utf8_htmlentities($item_info['name'])?></a> <span class="stufe">(Level&nbsp;<?php echo ths($item_info['level'])?>)</span></h3>
<?php
...
<?php
        ...
        {
?>
    <div class="restbauzeit" id="restbauzeit-<?php echo htmlentities($id)?>">Fertigstellung: <?php echo date('H:i:s, Y-m-d', $building[1])?> (Serverzeit), <a href="forschung.php?cancel=<?php echo htmlentities(urlencode($id))?>&amp;<?php echo htmlentities(urlencode(session_name()).'='.urlencode(session_id()))?>" class="abbrechen">Abbrechen</a></div>
...
?>
    <dl>
        ...
        <dd class="item-bauzeit forschung-lokal"><?php echo format_btime($item_info['time_local'])?></dd>

        <dt class="item-bauzeit forschung-global">Bauzeit global</dt>
        <dd class="item-bauzeit forschung-global"><?php echo format_btime($item_info['time_global'])?></dd>
    </dl>
</div>
<?php
    }
