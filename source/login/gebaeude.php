...
<?php
    ...
    foreach($gebaeude as $id)
    {
        ...
        if(!$geb['deps-okay'] && $geb['level'] <= 0) # Abhaengigkeiten nicht erfuellt
            continue;
?>
<div class="item gebaeude" id="item-<?php echo htmlentities($id)?>">
    <h3><a href="help/description.php?id=<?php echo htmlentities(urlencode($id))?>&amp;<?php echo htmlentities(urlencode(session_name()).'='.urlencode(session_id()))?>" title="Genauere Informationen anzeigen"><?php echo utf8_htmlentities($geb['name'])?></a> <span class="stufe">(Stufe&nbsp;<?php echo ths($geb['level'])?>)</span></h3>
<?php
...
?>
        <li class="item-ausbau<?php echo $enough_ress ? '' : ' no-ress'?>"><?php echo $enough_ress ? '<a href="gebaeude.php?ausbau='.htmlentities(urlencode($id)).'&amp;'.htmlentities(urlencode(session_name()).'='.urlencode(session_id())).'" tabindex="'.($tabindex++).'">' : ''?>Ausbau auf Stufe&nbsp;<?php echo ths($geb['level']+1)?><?php echo $enough_ress ? '</a>' : ''?></li>
<?php
            }
            if($geb['debuildable'])
            {
...
?>
        <li class="item-rueckbau<?php echo $enough_ress ? '' : ' no-ress'?>"><?php echo $enough_ress ? '<a href="gebaeude.php?abbau='.htmlentities(urlencode($id)).'&amp;'.htmlentities(urlencode(session_name()).'='.urlencode(session_id())).'">' : ''?>Rückbau auf Stufe&nbsp;<?php echo ths($geb['level']-1)?><?php echo $enough_ress ? '</a>' : ''?></li>
<?php
            }
?>
    </ul>
<?php
        ...
        {
?>
    <div class="restbauzeit" id="restbauzeit-<?php echo htmlentities($building[0])?>">Fertigstellung: <?php echo date('H:i:s, Y-m-d', $building[1])?> (Serverzeit), <a href="gebaeude.php?cancel=<?php echo htmlentities(urlencode($building[0]))?>&amp;<?php echo htmlentities(urlencode(session_name()).'='.urlencode(session_id()))?>" class="abbrechen">Abbrechen</a></div>
...
?>
    <dl>
        ...
        <dd class="item-kosten">
            <?php echo format_ress($geb['ress'], 3)?>
        </dd>

        <dt class="item-bauzeit">Bauzeit</dt>
        <dd class="item-bauzeit"><?php echo format_btime($geb['time'])?></dd>
    </dl>
</div>
<?php
    }
...
<?php
        ...
        if($fastbuild_prev !== false)
        {
            $me->setActivePlanet($fastbuild_prev);
?>
    <li class="c-voriger2"><a href="gebaeude.php?planet=<?php echo htmlentities(urlencode($fastbuild_prev))?>&amp;<?php echo htmlentities(urlencode(session_name()).'='.urlencode(session_id()))?>" title="Voriger unbeschäftigter Planet: &bdquo;<?php echo utf8_htmlentities($me->planetName())?>&ldquo; (<?php echo utf8_htmlentities($me->getPosString())?>) [U]" rel="prev">&larr;</a></li>
<?php
        }
        if($fastbuild_next !== false)
        {
            $me->setActivePlanet($fastbuild_next);
?>
    <li class="c-naechster2"><a href="gebaeude.php?planet=<?php echo htmlentities(urlencode($fastbuild_next))?>&amp;<?php echo htmlentities(urlencode(session_name()).'='.urlencode(session_id()))?>" title="Nächster unbeschäftigter Planet: &bdquo;<?php echo utf8_htmlentities($me->planetName())?>&ldquo; (<?php echo utf8_htmlentities($me->getPosString())?>) [Q]" rel="next">&rarr;</a></li>
<?php
        }
        $me->setActivePlanet($active_planet);
?>
