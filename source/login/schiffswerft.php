...
?>
    <li class="c-voriger"><a href="schiffswerft.php?planet=<?php echohtmlentities(urlencode($fastbuild_prev))?>&amp;<?php echohtmlentities(urlencode(session_name()).'='.urlencode(session_id()))?>" title="Voriger unbeschäftigter Planet: &bdquo;<?php echoutf8_htmlentities($me->planetName())?>&ldquo; (<?php echoutf8_htmlentities($me->getPosString())?>) [U]" tabindex="1" accesskey="u" rel="prev">&larr;</a></li>
<?php
        }
...
?>
    <li class="c-naechster"><a href="schiffswerft.php?planet=<?php echohtmlentities(urlencode($fastbuild_next))?>&amp;<?php echohtmlentities(urlencode(session_name()).'='.urlencode(session_id()))?>" title="Nächster unbeschäftigter Planet: &bdquo;<?php echoutf8_htmlentities($me->planetName())?>&ldquo; (<?php echoutf8_htmlentities($me->getPosString())?>) [Q]" tabindex="2" accesskey="q" rel="next">&rarr;</a></li>
<?php
        }
        $me->setActivePlanet($active_planet);
?>
...
<form action="schiffswerft.php?<?php echohtmlentities(urlencode(session_name()).'='.urlencode(session_id()))?>" method="post">
<?php
    $tabindex = 1;
    ...
    foreach($schiffe as $id)
    {
        ...
        if(!$item_info['buildable'] && $item_info['level'] <= 0)
            continue;
?>
    <div class="item schiffe" id="item-<?php echohtmlentities($id)?>">
...
?>
        <ul>
            <li class="item-bau"><input type="text" name="schiffe[<?php echoutf8_htmlentities($id)?>]" value="0" tabindex="<?php echo$tabindex++?>" /></li>
        </ul>
<?php
        }
?>
...
<form action="<?php echohtmlentities(global_setting("USE_PROTOCOL").'://'.$_SERVER['HTTP_HOST'].h_root.'/login/schiffswerft.php?'.urlencode(session_name()).'='.urlencode(session_id()))?>" method="post" class="alle-abbrechen">
