...
?>
<div class="desc">
    <h2><?php echo utf8_htmlentities($item->getInfo('name'))?><?php if($lvl >= 0){?> <span class="stufe">(Stufe&nbsp;<?php echo utf8_htmlentities($lvl)?>)</span><?php }?></h2>
<?php
...
<dl class="item-info">
    ...
    <dd class="item-kosten">
        <?php echo format_ress($item_info['ress'], 3)?>
    </dd>
...
?>
    ...
    <dd class="item-bauzeit-lokal"><?php echo format_btime($item_info['time_local'])?></dd>

    <dt class="item-bauzeit-global">Bauzeit global</dt>
    <dd class="item-bauzeit-global"><?php echo format_btime($item_info['time_global'])?></dd>
<?php
            }
            else
            {
    ...
    <table>
        ...
        <tbody>
            <tr>
                ...
                <td><?php echo utf8_htmlentities($item->getInfo('fields'))?></td>
            </tr>
        </tbody>
    </table>
</div>
...
?>
            <tr<?php echo ($act_lvl == $lvl) ? ' class="active"' : ''?>>
                <th><?php echo ths($act_lvl)?></th>
<?php
                    if($prod[0] != 0)
                    {
?>
                <td class="c-carbon"><?php echo ths($prod[0]*pow($act_lvl, 2))?></td>
<?php
                    }
                    if($prod[1] != 0)
                    {
?>
                <td class="c-aluminium"><?php echo ths($prod[1]*pow($act_lvl, 2))?></td>
<?php
                    }
                    if($prod[2] != 0)
                    {
?>
                <td class="c-wolfram"><?php echo ths($prod[2]*pow($act_lvl, 2))?></td>
<?php
                    }
                    if($prod[3] != 0)
                    {
?>
                <td class="c-radium"><?php echo ths($prod[3]*pow($act_lvl, 2))?></td>
<?php
                    }
                    if($prod[4] != 0)
                    {
?>
                <td class="c-tritium"><?php echo ths($prod[4]*pow($act_lvl, 2))?></td>
<?php
                    }
                    if($prod[5] != 0)
                    {
?>
                <td class="c-energie"><?php echo ths($prod[5]*pow($act_lvl, 2))?></td>
<?php
                    }
?>
            </tr>
    ...
    <table>
        ...
        <tbody>
            <tr>
                ...
                <td><?php $rueckstoss_info = $me->getItemInfo('F6', 'forschung');
...
echo ths($trans[0])."&nbsp;(".ths(round($trans[0]* pow(1.2, $lade_info['level']))); ?>)&nbsp;Tonnen</td>
            ...
            <tr>
                ...
                <td><?php echo ths($item->getInfo('att'))."&nbsp;(".ths(round($item->getInfo('att')*pow(1.05,$angriff_info['level'])))?>)</td>
            </tr>
            <tr>
                <th>Schild</th>
                <td><?php echo ths($item->getInfo('def'))."&nbsp;(".ths(round($item->getInfo('def')*pow(1.05,$def_info['level'])))?>)</td>
            </tr>
            <tr>
                <th>Antriebsstärke</th>
                
                <td><?php $speedy=$item->getInfo('speed');
...
<?php
            foreach($item->getInfo('types') as $t)
            {
                if(isset($type_names[$t]))
                    $t = $type_names[$t];
?>
                        <li><?php echo utf8_htmlentities($t)?></li>
<?php
            }
?>
                    </ul>
