...
<form action="verteidigung.php?<?php echo htmlentities(urlencode(session_name()).'='.urlencode(session_id()))?>" method="post">
<?php
    $tabindex = 1;
    ...
    foreach($verteidigung as $id)
    {
        ...
        if(!$item_info['buildable'] && $item_info['level'] <= 0)
            continue;
?>
    <div class="item verteidigung" id="item-<?php echo htmlentities($id)?>">
...
?>
        <ul>
            <li class="item-bau"><input type="text" name="verteidigung[<?php echo utf8_htmlentities($id)?>]" value="0" tabindex="<?php echo $tabindex++?>" /></li>
        </ul>
<?php
        }
?>
...
<form action="<?php echo htmlentities(global_setting("USE_PROTOCOL").'://'.$_SERVER['HTTP_HOST'].h_root.'/login/verteidigung.php?'.urlencode(session_name()).'='.urlencode(session_id()))?>" method="post" class="alle-abbrechen">
