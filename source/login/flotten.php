...
if(isset($_GET['action_galaxy']) && isset($_GET['action_system']) && isset($_GET['action_planet']) && isset($_GET['action']) && ($_GET['action'] == 'spionage' || $_GET['action'] == 'besiedeln' || $_GET['action'] == 'sammeln' || $_GET['action'] == 'shortcut'))
{
    if($_GET['action'] == 'shortcut')
    {
        ...
        {
            ?>
<p class="<?php echo htmlspecialchars($return[0])?>"><?php echo htmlspecialchars($return[1])?></p>
            <?php
            login_gui::html_foot();
            exit();
        }
    ...
    if($my_flotten >= $max_flotten)
    {
        ...
        ?>
<p class="error">Maximale Flottenzahl erreicht.</p>
        <?php
        login_gui::html_foot();
        exit();
    }
...
<p class="error">Das Versenden von Flotten ist im Zusammenhang mit dem
Demo-Account nicht moeglich.</p>

        <?php
        login_gui::html_foot();
        exit();
    }
    else if ( IsGameOperator( $planet_owner ) )
    {
        if( defined('ajax') )
        return array('error', 'GO-Account');
        ?>

<p class="error">Das Versenden von Flotten ist im Zusammenhang mit einem
GameOperator-Account nicht moeglich.</p>
    ...
    else
    {
        ...
        if($_GET['action'] == 'spionage')
        {
            ...
            if($me->getItemLevel('S5', 'schiffe') < 1)
            {
                ...
                ?>
<p class="error">Keine Spionagesonden vorhanden.</p>
                <?php
                login_gui::html_foot();
                exit();
            }
        ...
        elseif($_GET['action'] == 'besiedeln')
        {
            ...
            if($me->getItemLevel('S6', 'schiffe') < 1)
            {
                ...
                ?>
<p class="error">Kein Besiedelungsschiff vorhanden.</p>
                <?php
                login_gui::html_foot();
                exit();
            }
        ...
        elseif($_GET['action'] == 'sammeln')
        {
            ...
            if($me->getItemLevel('S3', 'schiffe') < 1)
            {
                ...
                ?>
<p class="error">Keine Sammler vorhanden.</p>
                <?php
            }
        }
    }
...
GameOperator-Account nicht moeglich.</p>
  		        ...
  		        else if( count( $types ) <= 0 )
  		        {
  		            ...
  		            ?>
<p class="error">Sie haben nicht die richtigen Schiffe ausgewählt, um
diesen Planeten anzufliegen.</p>
  		            <?php
  		        }
  		        else
  		        {
  		                ...
  		                else
  		                {
  		                    ...
  		                    if($planet_owner && ($_POST['auftrag'] == '3' || $_POST['auftrag'] == '5') && !$that_user->userLocked() && !file_exists(global_setting("DB_NONOOBS")))
  		                    {
  		                        ...
  		                        if($that_punkte < 2500 || $this_punkte < 2500)
  		                        {
  		                            ...
  		                            ?>
<p class="error">Es ist eine intergalaktische Störung aufgetreten! Die
Sensoren sind beim dem Versuch, einen Anflugspunkt auszumachen,
durcheinandergekommen.(<abbr title="Also known as" xml:lang="en">Aka</abbr>
Anfängerschutz.)</p>
  		                            <?php
  		                            $noob = true;
  		                        }
  		                        elseif(($that_punkte < 5000 && $this_punkte > $that_punkte*1.3) || ($this_punkte < 5000 && $that_punkte > $this_punkte*1.3))
  		                        {
  		                            if(defined('ajax')) return array('error', 'Noobschutz.');
  		                            ?>
<p class="error">Es ist eine intergalaktische Störung aufgetreten!<br>
Die Sensoren sind beim dem Versuch, einen Anflugspunkt auszumachen,
durcheinandergekommen.(<abbr title="Also known as" xml:lang="en">Aka</abbr>
Anfängerschutz.)</p>
  		                            <?php
  		                            $noob = true;
  		                            ...
  		                            if(($_POST['auftrag'] == 4))
  		                            {
  		                                ...
  		                                foreach($fleets as $fleet)
  		                                {
  		                                    ...
  		                                    if(in_array($_SESSION['username'], $username1) && ($hold > 0) && ($target[0] == $target1[0]) && $type == 3)
  		                                    {
  		                                        ...
  		                                        ?>
<p class="error">Sie können diesen Spieler nicht zum Halten anfliegen,
da eigene Flotten mit Angriffsauftrag zum Planeten unterwegs sind.</p>
  		                                        <?php
  		                            ...
  		                            elseif($_POST['auftrag'] == 3)
  		                            {
  		                                ...
  		                                foreach($fleets as $fleet)
  		                                {
  		                                    ...
  		                                    if(in_array($_SESSION['username'], $username1) && ($hold != 0) && ($target[0] == $target1[0]) && $type == 4)
  		                                    {
  		                                        if(defined('ajax')) return array('error', 'Kein Angriff auf eigene Halteflotten!!');
  		                                        ?>
<p class="error">Sie können diesen Spieler nicht angreifen, da eigene
Flotten mit Halteauftrag zum Planeten unterwegs sind, oder sich bereits
dort befinden.</p>
  		                                        <?php
  		                            ...
  		                            {
  		                                ?>
<p class="error">Geht bis zur Umstellung des Kampfsystems nicht!!!</p>
  		                                <?php
  		                            }
  		                            ...
  		                            elseif($ress[4]-$_POST['transport'][4] < $tritium)
  		                            {
  		                                ...
  		                                ?>
<p class="error">Nicht genug Tritium vorhanden.</p>
  		                                <?php
  		                            }
  		                            elseif($buendnisflug && !$bndfleet_obj->getBndUsersCount($_SESSION['username']))
  		                            {
  		                                ?>
<p class="error">Die maximale Verbandsuseranzahl wurde bereits erreicht.
Sie müssen woanders mitfliegen oder zuhause bleiben.</p>
  		                                <?php
  		                                $fleet_obj->destroy();
  		                            }
  		                            ...
  		                            {
  		                                ?>
<p class="error">Ihre Flotte ist zu langsam, um am Verband teilzunehmen.
</p>
  		                                <?php
  		                                $fleet_obj->destroy();
  		                            }
...
if($buendnisflug)
{
	...
	<dd class="c-ziel"><?php echo utf8_htmlentities($bndfleet_obj->getCurrentTarget())?>
	&ndash; <?php echo $planet_owner ? utf8_htmlentities($galaxy_obj->getPlanetName($_POST['system'], $_POST['planet'])).' <span class="playername">('.utf8_htmlentities($planet_owner).')</span>' : 'Unbesiedelt'?></dd>


	<dt class="c-auftragsart">Auftragsart</dt>
	<dd class="c-auftragsart">Verbandsangriff
	
	
	</dt>

	<dt class="c-ankunft">Ankunft</dt>
	<dd class="c-ankunft"><?php echo date('H:i:s, Y-m-d', $bndfleet_obj->getArrivalTime())?>
	(Serverzeit)</dd>
	<?php
}
if(!$buendnisflug)
{
    ?>
	<dt class="c-ziel">Ziel</dt>
	<dd class="c-ziel"><?php echo utf8_htmlentities($fleet_obj->getCurrentTarget())?>
	&ndash; <?php echo $planet_owner ? utf8_htmlentities($galaxy_obj->getPlanetName($_POST['system'], $_POST['planet'])).' <span class="playername">('.utf8_htmlentities($planet_owner).')</span>' : 'Unbesiedelt'?></dd>


	<dt class="c-auftragsart">Auftragsart</dt>
	<dd class="c-auftragsart"><?php echo isset($type_names[$_POST['auftrag']]) ? htmlentities($type_names[$_POST['auftrag']]) : utf8_htmlentities($_POST['auftrag'])?>
	
	
	</dt>

	<dt class="c-ankunft">Ankunft</dt>
	<dd class="c-ankunft"><?php echo date('H:i:s, Y-m-d', $fleet_obj->getArrivalTime())?>
	(Serverzeit)</dd>
	<?php
}
if($_POST['haltezeit'] !=0)
{
	...
	<dd class="c-haltezeit"><?php echo ($_POST['haltezeit'])." Minuten"?></dd>
	<?php
}
if(isset($_POST['saveflug']))
{
	...
	<dd class="c-saveflug"><?php echo "Saveflug"?></dd>
	<?php
}
?>
...
<?php
  		            ...
  		            if($show_form2)
  		            {
  		                ...
  		                if($fleet_obj->create())
  		                {
  		                    ...
  		                    ?>
<form
	action="flotten.php?<?php echo htmlentities(urlencode(session_name()).'='.urlencode(session_id()))?>"
...
<dl>
	<dt class="c-ziel">Ziel</dt>
	<dd class="c-ziel"><?php echo utf8_htmlentities($_POST['galaxie'].':'.$_POST['system'].':'.$_POST['planet'])?>
	&ndash; <?php echo $planet_owner ? utf8_htmlentities($galaxy_obj->getPlanetName($_POST['system'], $_POST['planet'])).' <span class="playername">('.utf8_htmlentities($galaxy_obj->getPlanetOwner($_POST['system'], $_POST['planet'])).')</span>' : 'Unbesiedelt'?></dd>

	<dt class="c-entfernung">Entfernung</dt>
	<dd class="c-entfernung"><?php echo ths($distance)?>&thinsp;<abbr
		title="Orbits">Or</abbr></dd>

	<dt class="c-antrieb">Antrieb</dt>
	<dd class="c-antrieb"><?php echo ths($speed)?>&thinsp;<abbr
		title="Mikroorbits pro Quadratsekunde">µOr&frasl;s²</abbr></dd>

	<dt class="c-tritiumverbrauch">Tritiumverbrauch</dt>
	<dd
		class="c-tritiumverbrauch <?php echo ($this_ress[4] >= $tritium) ? 'ja' : 'nein'?>"
		id="tritium-verbrauch"><?php echo ths($tritium)?>&thinsp;<abbr title="Tonnen">t</abbr></dd>
	...
	<script type="text/javascript" src="<?php echo htmlentities(h_root.'/javascript/yui-min.js')?>"></script>
	...
	<dd class="c-flugzeit" id="flugzeit"
		title="Ankunft: <?php echo date('H:i:s, Y-m-d', time()+$time)?> (Serverzeit)"><?php echo $time_string?></dd>
		<?php
		if($buendnisflug)
		{
	...
	<dd
		class="c-tritiumverbrauch <?php echo ($this_ress[4] >= $tritium) ? 'ja' : 'nein'?>"
		id="restbauzeit-<?php echo utf8_htmlentities($buendnisflug_id)?>">Ankunft: <?php echo date('H:i:s, Y-m-d', $arrival_time1)?>
	(Serverzeit)</dd>
	<script type="text/javascript">	
	init_countdown('<?php echo $countdown1[0]?>', <?php echo $countdown1[1]?>, <?php echo $countdown1[2] ? 'false' : 'true'?>, <?php echo global_setting("EVENTHANDLER_INTERVAL")?>);		
	</script>
	</dd>
	<dt class="c-auftrag"><label for="auftrag">A<kbd>u</kbd>ftrag</label></dt>
	<dd class="c-auftrag"><legend>Verbandsangriff</legend></dd>
	<?php
		}
		?>

	<dt class="c-transportkapazitaet">Transportkapazität</dt>
	<dd class="c-transportkapazitaet"><?php echo ths($transport[0])?>&thinsp;<abbr
	...
	<script type="text/javascript">
			...
			document.write('<dd class="c-verbleibend" id="transport-verbleibend-dd"><?php echo ths($transport[0])?>&thinsp;<abbr title="Tonnen">t</abbr></dd>');
			// ]]>
		</script>
	<dt class="c-auftrag"><label for="auftrag">A<kbd>u</kbd>ftrag</label></dt>
	<dd class="c-auftrag"><select name="auftrag" id="auftrag" accesskey="u"
		tabindex="2" onchange="recalc_values();" onkeyup="recalc_values();">
		<?php
		foreach($types as $type)
		{
		    ?>
		<option value="<?php echo utf8_htmlentities($type)?>"><?php echo isset($type_names[$type]) ? htmlentities($type_names[$type]) : $type?></option>
		<?php
		}
		?>
	</select></dd>
	...
	if($transport[0] > 0)
	{
		...
		<dt><label for="transport-carbon">Carbo<kbd>n</kbd></label> - <a
			href="#"
			onclick='document.getElementById("transport-carbon").value = "<?php echo ereg_replace("&nbsp;","",ths($ress[0]))?>";recalc_values();'
		...
		<dt><label for="transport-aluminium">Aluminium</label> - <a href="#"
			onclick='document.getElementById("transport-aluminium").value = "<?php echo ereg_replace("&nbsp;","",ths($ress[1]))?>";recalc_values();'
		...
		<dt><label for="transport-wolfram">Wolfram</label> - <a href="#"
			onclick='document.getElementById("transport-wolfram").value = "<?php echo ereg_replace("&nbsp;","",ths($ress[2]))?>";recalc_values();'
		...
		<dt><label for="transport-radium">Radium</label> - <a href="#"
			onclick='document.getElementById("transport-radium").value = "<?php echo ereg_replace("&nbsp;","",ths($ress[3]))?>";recalc_values();'
		...
		<dt><label for="transport-tritium">Tritium</label> - <a href="#"
			onclick='document.getElementById("transport-tritium").value = "<?php echo ereg_replace("&nbsp;","",ths($ress[4]))-ereg_replace("&nbsp;","",ths($tritium+2))?>";recalc_values();'
	...
	if($transport[1] > 0)
	{
	    ...
	    foreach($me->getItemsList('roboter') as $rob)
	    {
	        ...
	        ?>
		<dt><label for="rtransport-<?php echo utf8_htmlentities($rob)?>"><?php echo utf8_htmlentities($item_info['name'])?></label></dt>
		<dd><input type="text" name="rtransport[<?php echo utf8_htmlentities($rob)?>]"
			id="rtransport-<?php echo utf8_htmlentities($rob)?>" value="0"
			onchange="recalc_values();" tabindex="<?php echo $tabindex++?>"
			onkeyup="recalc_values();" onclick="recalc_values();" /></dd>
			<?php
	    }
	}
	...
	<?php
		...
		if(!$buendnisflug && ($transport[0] > 0 || $transport[1] > 0))
		{
		    ?>
	<dl>
		<dt class="c-transport1" id="transport1-dt"><label
			for="transport1-haltezeit-dt">Haltezeit</label></dt>
		<dd class="c-transport1" id="transport1-dd"><input type="text"
			name="haltezeit" id="haltezeit" value="0" onchange="recalc_values();"
			accesskey="n" tabindex="8" onkeyup="recalc_values();"
			onclick="recalc_values();" /></dd>

		<dt class="c-transport2" id="transport2-dt"><label
			for="transport2-saveflug-dt">Saveflug</label></dt>
		<dd class="c-transport2" id="transport2-dd"><input type="checkbox"
			name="saveflug" id="saveflug" value="1"><br>
		</dd>
	</dl>
...
?>
				...
				document.getElementById('tritium-verbrauch').className = 'c-tritiumverbrauch '+((<?php echo $this_ress[4]?> >= tritium) ? 'ja' : 'nein');
...
?>
					var ges_rob = <?php echo implode('+', $robs_arr)?>;
<?php
								}
								else
								{
...
?>
					var remain_ress = <?php echo $transport[0]?>;
					if(!isNaN(ges_ress))
						remain_ress -= ges_ress;
					var remain_rob = <?php echo $transport[1]?>;
					if(!isNaN(ges_rob))
						remain_rob -= ges_rob;
					if(remain_ress < 0)
						remain_ress = "\u22120";
...
foreach($_POST['flotte'] as $id=>$anzahl)
{
    ?> <input type="hidden" name="flotte[<?php echo utf8_htmlentities($id)?>]"
	value="<?php echo utf8_htmlentities($anzahl)?>" /> <?php
}
?> <input type="hidden" name="galaxie"
	value="<?php echo utf8_htmlentities($_POST['galaxie'])?>" /> <input
	type="hidden" name="system"
	value="<?php echo utf8_htmlentities($_POST['system'])?>" /> <input
	type="hidden" name="planet"
	value="<?php echo utf8_htmlentities($_POST['planet'])?>" /> <?php
	if($buendnisflug)
	{
	    $_POST["buendnis_benutzername1"] = ereg_replace(' ', "&nbsp", $_POST["buendnis_benutzername"]);
	    ?> <!--<button type="submit" accesskey="d" name="buendnisflug1" value=<?php echo $_POST["buendnis_benutzername"]?>>Absen<kbd>d</kbd>en</button>-->
<button type="submit" accesskey="d" name="buendnisflug1"
	value=<?php echo $_POST["buendnis_benutzername1"]."/".$_POST["buendnis_flottenpasswort"]?>>Absen<kbd>d</kbd>en</button>
...
<p
	class="flotte-anzahl<?php echo ($my_flotten >= $max_flotten) ? ' voll' : ''?>">
Sie haben derzeit <?php echo ths($my_flotten)?> von <?php echo ths($max_flotten)?> <?php echo ($max_flotten == 1) ? 'möglichen Flotte' : 'möglichen Flotten'?>
unterwegs.<br />
...
<form
	...
	{
	    ?>
<fieldset class="flotte-koords"><legend><input type="radio"
	name="buendnisflug" value="0" <?php if(!$buendnisflug){?>
	checked="checked" <?php }?> id="i-eigenes-ziel" /> <label
	for="i-eigenes-ziel">Eigenes Ziel</label></legend>
<dl>
	...
	<dd class="c-ziel"><input type="text" id="ziel-galaxie" name="galaxie"
		value="<?php echo utf8_htmlentities($this_pos[0])?>" title="Ziel: Galaxie"
		accesskey="z" tabindex="1" onclick="syncronise(true);"
		onchange="syncronise(true);" onkeyup="syncronise(true);"
		maxlength="<?php echo strlen(getGalaxiesCount())?>" />:<input type="text"
		id="ziel-system" name="system"
		value="<?php echo utf8_htmlentities($this_pos[1])?>" title="Ziel: System"
		...
		value="<?php echo utf8_htmlentities($this_pos[2])?>" title="Ziel: Planet"
...
?>
					document.write('<option value="<?php echo htmlspecialchars($shortcut)?>"><?php echo preg_replace('/[\'\\\\]/', '\\\\\\0', htmlspecialchars($s))?></option>');
<?php
					}
?>
...
?>
					document.write('<option value="<?php echo utf8_htmlentities($me->getPosString())?>"<?php echo ($planet == $active_planet) ? ' selected="selected"' : ''?>><?php echo utf8_htmlentities($me->getPosString())?>: <?php echo preg_replace('/[\'\\\\]/', '\\\\\\0', utf8_htmlentities($me->planetName()))?></option>');
<?php
				}
...
<?php
				...
				{
?>
					document.write('<a href="flotten_actions.php?action=shortcuts&amp;<?php echo htmlspecialchars(urlencode(session_name()).'='.urlencode(session_id()))?>" class="lesezeichen-verwalten-link">[Lesezeichen verwalten]</a>');
<?php
				}
?>
					document.write('</dd>');
...
<fieldset class="buendnisflug"><legend><input type="radio"
	name="buendnisflug" value="1" <?php if($buendnisflug){?>
	checked="checked" <?php }?> id="i-buendnisflug" /> <label
	for="i-buendnisflug">Bündnisflug</label></legend>
<dl>
	...
	<dd class="c-benutzername"><input type="text"
		...
		value="<?php echo htmlspecialchars($_POST["buendnis_benutzername"])?>"
		<?php }?> /></dd>
	...
	<dd class="c-passwort"><input type="text"
		...
		value="<?php echo htmlspecialchars($_POST["buendnis_flottenpasswort"])?>"
		<?php }?> /></dd>
</dl>
</fieldset>
<script type="text/javascript">
...
foreach($me->getItemsList('schiffe') as $id)
{
    ...
    ?>
	<dt><a
		href="help/description.php?id=<?php echo htmlentities(urlencode($id))?>&amp;<?php echo htmlentities(urlencode(session_name()).'='.urlencode(session_id()))?>"
		title="Genauere Informationen anzeigen"><?php echo utf8_htmlentities($item_info['name'])?></a>
	<span class="vorhanden">(<?php echo ths($item_info['level'])?>&nbsp;vorhanden)</span></dt>
	<dd><input type="text" name="flotte[<?php echo utf8_htmlentities($id)?>]"
		value="0" tabindex="<?php echo $i?>"
		<?php echo ($my_flotten >= $max_flotten || !$me->permissionToAct()) ? ' readonly="readonly"' : ''?>
		id="<?php echo $i?>" /> <input name="button" type="button"
		onclick='document.getElementById("<?php echo $i?>").value = "<?php echo $item_info['level']?>"'
		ondblclick='document.getElementById("<?php echo $i?>").value = "0"'
		value="max" /></dd>
		<?php
		$i++;
}
...
if($i>5 && $my_flotten < $max_flotten && $me->permissionToAct())
{
    ?>
<div>
<button type="submit" accesskey="w" tabindex="<?php echo $i?>"><kbd>W</kbd>eiter</button>
</div>
    <?php
}
?></form>
