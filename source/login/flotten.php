...
if(isset($_GET['action_galaxy']) && isset($_GET['action_system']) && isset($_GET['action_planet']) && isset($_GET['action']) && ($_GET['action'] == 'spionage' || $_GET['action'] == 'besiedeln' || $_GET['action'] == 'sammeln' || $_GET['action'] == 'shortcut'))
{
    if($_GET['action'] == 'shortcut')
    {
        ...
        {
            ?>
<p class="<?php echohtmlspecialchars($return[0])?>"><?php echohtmlspecialchars($return[1])?></p>
            <?php
            login_gui::html_foot();
            exit();
        }
...
if($buendnisflug)
{
	...
	<dd class="c-ziel"><?php echoutf8_htmlentities($bndfleet_obj->getCurrentTarget())?>
	...
	<dd class="c-ankunft"><?php echodate('H:i:s, Y-m-d', $bndfleet_obj->getArrivalTime())?>
	(Serverzeit)</dd>
	<?php
}
if(!$buendnisflug)
{
    ?>
	<dt class="c-ziel">Ziel</dt>
	<dd class="c-ziel"><?php echoutf8_htmlentities($fleet_obj->getCurrentTarget())?>
	&ndash; <?php echo$planet_owner ? utf8_htmlentities($galaxy_obj->getPlanetName($_POST['system'], $_POST['planet'])).' <span class="playername">('.utf8_htmlentities($planet_owner).')</span>' : 'Unbesiedelt'?></dd>


	<dt class="c-auftragsart">Auftragsart</dt>
	<dd class="c-auftragsart"><?php echoisset($type_names[$_POST['auftrag']]) ? htmlentities($type_names[$_POST['auftrag']]) : utf8_htmlentities($_POST['auftrag'])?>
	
	
	</dt>

	<dt class="c-ankunft">Ankunft</dt>
	<dd class="c-ankunft"><?php echodate('H:i:s, Y-m-d', $fleet_obj->getArrivalTime())?>
	(Serverzeit)</dd>
	<?php
}
...
<?php
  		            ...
  		            if($show_form2)
  		            {
  		                ...
  		                if($fleet_obj->create())
  		                {
  		                    ...
  		                    ?>
<form
	action="flotten.php?<?php echohtmlentities(urlencode(session_name()).'='.urlencode(session_id()))?>"
...
<dl>
	<dt class="c-ziel">Ziel</dt>
	<dd class="c-ziel"><?php echoutf8_htmlentities($_POST['galaxie'].':'.$_POST['system'].':'.$_POST['planet'])?>
	...
	<dd class="c-entfernung"><?php echoths($distance)?>&thinsp;<abbr
		title="Orbits">Or</abbr></dd>

	<dt class="c-antrieb">Antrieb</dt>
	<dd class="c-antrieb"><?php echoths($speed)?>&thinsp;<abbr
	...
	<dd
		...
		id="tritium-verbrauch"><?php echoths($tritium)?>&thinsp;<abbr title="Tonnen">t</abbr></dd>
	...
	<script type="text/javascript" src="<?php echohtmlentities(h_root.'/javascript/yui-min.js')?>"></script>
	...
	<dd class="c-flugzeit" id="flugzeit"
		title="Ankunft: <?php echodate('H:i:s, Y-m-d', time()+$time)?> (Serverzeit)"><?php echo$time_string?></dd>
		<?php
		if($buendnisflug)
		{
	...
	<dd
		class="c-tritiumverbrauch <?php echo($this_ress[4] >= $tritium) ? 'ja' : 'nein'?>"
		id="restbauzeit-<?php echoutf8_htmlentities($buendnisflug_id)?>">Ankunft: <?php echodate('H:i:s, Y-m-d', $arrival_time1)?>
	(Serverzeit)</dd>
	<script type="text/javascript">	
	init_countdown('<?php echo$countdown1[0]?>', <?php echo$countdown1[1]?>, <?php echo$countdown1[2] ? 'false' : 'true'?>, <?php echoglobal_setting("EVENTHANDLER_INTERVAL")?>);		
	</script>
	</dd>
	...
	<dd class="c-transportkapazitaet"><?php echoths($transport[0])?>&thinsp;<abbr
	...
	<script type="text/javascript">
			...
			document.write('<dd class="c-verbleibend" id="transport-verbleibend-dd"><?php echoths($transport[0])?>&thinsp;<abbr title="Tonnen">t</abbr></dd>');
			// ]]>
		</script>
	<dt class="c-auftrag"><label for="auftrag">A<kbd>u</kbd>ftrag</label></dt>
	<dd class="c-auftrag"><select name="auftrag" id="auftrag" accesskey="u"
		...
		{
		    ?>
		<option value="<?php echoutf8_htmlentities($type)?>"><?php echoisset($type_names[$type]) ? htmlentities($type_names[$type]) : $type?></option>
		<?php
		}
		?>
	</select></dd>
	...
	if($transport[0] > 0)
	{
		...
		<dt><label for="transport-carbon">Carbo<kbd>n</kbd></label> - <a
			href="#"
			onclick='document.getElementById("transport-carbon").value = "<?php echoereg_replace("&nbsp;","",ths($ress[0]))?>";recalc_values();'
		...
		<dt><label for="transport-aluminium">Aluminium</label> - <a href="#"
			onclick='document.getElementById("transport-aluminium").value = "<?php echoereg_replace("&nbsp;","",ths($ress[1]))?>";recalc_values();'
		...
		<dt><label for="transport-wolfram">Wolfram</label> - <a href="#"
			onclick='document.getElementById("transport-wolfram").value = "<?php echoereg_replace("&nbsp;","",ths($ress[2]))?>";recalc_values();'
		...
		<dt><label for="transport-radium">Radium</label> - <a href="#"
			onclick='document.getElementById("transport-radium").value = "<?php echoereg_replace("&nbsp;","",ths($ress[3]))?>";recalc_values();'
		...
		<dt><label for="transport-tritium">Tritium</label> - <a href="#"
			onclick='document.getElementById("transport-tritium").value = "<?php echoereg_replace("&nbsp;","",ths($ress[4]))-ereg_replace("&nbsp;","",ths($tritium+2))?>";recalc_values();'
	...
	if($transport[1] > 0)
	{
	    ...
	    foreach($me->getItemsList('roboter') as $rob)
	    {
	        ...
	        ?>
		<dt><label for="rtransport-<?php echoutf8_htmlentities($rob)?>"><?php echoutf8_htmlentities($item_info['name'])?></label></dt>
		<dd><input type="text" name="rtransport[<?php echoutf8_htmlentities($rob)?>]"
			id="rtransport-<?php echoutf8_htmlentities($rob)?>" value="0"
...
?>
					var ges_rob = <?php echoimplode('+', $robs_arr)?>;
<?php
								}
								else
								{
...
foreach($_POST['flotte'] as $id=>$anzahl)
{
    ?> <input type="hidden" name="flotte[<?php echoutf8_htmlentities($id)?>]"
	value="<?php echoutf8_htmlentities($anzahl)?>" /> <?php
}
?> <input type="hidden" name="galaxie"
	value="<?php echoutf8_htmlentities($_POST['galaxie'])?>" /> <input
	type="hidden" name="system"
	value="<?php echoutf8_htmlentities($_POST['system'])?>" /> <input
	type="hidden" name="planet"
	value="<?php echoutf8_htmlentities($_POST['planet'])?>" /> <?php
	if($buendnisflug)
	{
...
Sie haben derzeit <?php echoths($my_flotten)?> von <?php echoths($max_flotten)?> <?php echo($max_flotten == 1) ? 'möglichen Flotte' : 'möglichen Flotten'?>
unterwegs.<br />
...
<dl>
	...
	<dd class="c-ziel"><input type="text" id="ziel-galaxie" name="galaxie"
		value="<?php echoutf8_htmlentities($this_pos[0])?>" title="Ziel: Galaxie"
		accesskey="z" tabindex="1" onclick="syncronise(true);"
		onchange="syncronise(true);" onkeyup="syncronise(true);"
		maxlength="<?php echostrlen(getGalaxiesCount())?>" />:<input type="text"
		id="ziel-system" name="system"
		value="<?php echoutf8_htmlentities($this_pos[1])?>" title="Ziel: System"
		...
		value="<?php echoutf8_htmlentities($this_pos[2])?>" title="Ziel: Planet"
...
?>
					document.write('<option value="<?php echohtmlspecialchars($shortcut)?>"><?php echopreg_replace('/[\'\\\\]/', '\\\\\\0', htmlspecialchars($s))?></option>');
<?php
					}
?>
...
?>
					document.write('<option value="<?php echoutf8_htmlentities($me->getPosString())?>"<?php echo($planet == $active_planet) ? ' selected="selected"' : ''?>><?php echoutf8_htmlentities($me->getPosString())?>: <?php echopreg_replace('/[\'\\\\]/', '\\\\\\0', utf8_htmlentities($me->planetName()))?></option>');
<?php
				}
...
<?php
				...
				{
?>
					document.write('<a href="flotten_actions.php?action=shortcuts&amp;<?php echohtmlspecialchars(urlencode(session_name()).'='.urlencode(session_id()))?>" class="lesezeichen-verwalten-link">[Lesezeichen verwalten]</a>');
<?php
				}
?>
					document.write('</dd>');
...
<dl>
	...
	<dd class="c-benutzername"><input type="text"
		...
		value="<?php echohtmlspecialchars($_POST["buendnis_benutzername"])?>"
		<?php }?> /></dd>
	...
	<dd class="c-passwort"><input type="text"
		...
		value="<?php echohtmlspecialchars($_POST["buendnis_flottenpasswort"])?>"
		<?php }?> /></dd>
</dl>
</fieldset>
<script type="text/javascript">
...
foreach($me->getItemsList('schiffe') as $id)
{
    ...
    ?>
	<dt><a
		href="help/description.php?id=<?php echohtmlentities(urlencode($id))?>&amp;<?php echohtmlentities(urlencode(session_name()).'='.urlencode(session_id()))?>"
		title="Genauere Informationen anzeigen"><?php echoutf8_htmlentities($item_info['name'])?></a>
	<span class="vorhanden">(<?php echoths($item_info['level'])?>&nbsp;vorhanden)</span></dt>
	<dd><input type="text" name="flotte[<?php echoutf8_htmlentities($id)?>]"
